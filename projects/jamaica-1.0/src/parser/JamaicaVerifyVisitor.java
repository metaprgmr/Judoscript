...
public class JamaicaVerifyVisitor extends JamaicaVisitorBase
{
  ...
  public Object visit(ASTClassDeclaration node, Object data) throws Exception {
    ...
    for (int i=0; i<len; ++i) {
      ...
      if (n instanceof ASTVariableDeclarator)
        checkConflict("Field", ((ASTVariableDeclarator)n).getName(), fields, n);
    }
  ...
  public Object visit(ASTMethodDeclaration node, Object data) throws Exception {
    ...
    for (int i=0; i<len; ++i) {
      ...
      if (n instanceof ASTLabel)
        checkConflict("Label", ((ASTLabel)n).getName(), labels, n);
    }
  ...
  public Object visit(ASTVariableDeclarator node, Object data) throws Exception {
    ...
    if (parent instanceof ASTFormalParameters || parent instanceof ASTMethodDeclaration)
      checkConflict("Variable", fld, locals, node);
    // Fields are collect already.
    return null;
  }
  ...
  public Object visit(ASTCodeWithText node, Object data) throws Exception {
    ...
    case Constants.JSR_W:     checkLabel(node.getText(), node.getLineNum()); break;
    ...
    case Constants.RET:       checkExistence("Variable", node.getText(), locals, node.getLineNum()); break;
    }
    return null;
  }

  public Object visit(ASTCodeWithTextInt node, Object data) throws Exception {
    if (node.getOpcode() == 132) // iinc
      checkExistence("Field", node.getText(), fields, node.getLineNum());
    return null;
  }
  ...
  public Object visit(ASTCodeMemberAccess node, Object data) throws Exception {
    if (node.isSelfAccess())
      checkExistence("Field", node.getName(), fields, node.getLineNum());
    return null;
  }
  ...
  public Object visit(ASTCodeSwitch node, Object data) throws Exception {
    ...
    for (int i=caseLabels.length-1; i>=0; --i)
      checkLabel(caseLabels[i], node.getCaseLabelLineNum(caseLabels[i]));
    return null;
  }
  ...
  public Object visit(ASTMacroSet node, Object data) throws Exception {
    ...
    if (val != null && val instanceof ASTMacroBase)
      checkVariableField((ASTMacroBase)val);
    return null;
  }
  ...
  public Object visit(ASTMacroIterate node, Object data) throws Exception {
    if (!node.isEnd()) {
      ...
      if (s != null)
        checkVariableField(s, node.getIterateVarLine());
    }
    return null;
  }
  ...
  final void checkVariableField(String name, int line) {
    if (!locals.contains(name) && !fields.contains(name))
      addError("Variable/field '" + name + "' at line " + line + " is not defined.");
  }
    ...
    if (var.index != null) {
      if (var.index instanceof JavaClassCreator.VarAccess) {
        ...
        for (int i=0; i<oa.length; ++i)
          if (oa[i] instanceof JavaClassCreator.VarAccess)
            checkVariableField((JavaClassCreator.VarAccess)oa[i]);
      }
    }
  }
